...
public interface BLMS_VEHICLEDET_REPO extends JpaRepository<BLMS_VEHICLE_DET_ENTITY, String> {
	...
	List<BLMS_VEHICLE_DET_ENTITY> getapproved();
	
	@Query(value = "select * from BLMS_VEHICLE_LOAN_DETAILS  where verify_flg = 'N' and del_flg ='N'", nativeQuery = true)
	List<BLMS_VEHICLE_DET_ENTITY> getnotapproved();
