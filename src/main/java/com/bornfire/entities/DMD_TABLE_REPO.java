...
public interface DMD_TABLE_REPO extends JpaRepository<DMD_TABLE, DMD_TABLE_IDclass> {
	...
	@Query(value = "SELECT * FROM DEMAND_TBL WHERE loan_acct_no =?1 AND FLOW_DATE =?2 AND FLOW_CODE = 'INDEM'", nativeQuery = true)
	...
	@Query(value = "WITH ControlDate AS (\r\n" + "    SELECT MAX(TRAN_DATE) AS TRAN_DATE\r\n"
			...
			+ "FinalResult AS (\r\n"
			+ "    SELECT loan_acct_no, flow_id, acct_name, flow_date, flow_code, amount FROM PRDEMData\r\n"
			+ "    UNION ALL\r\n"
			+ "    SELECT loan_acct_no, flow_id, acct_name, flow_date, flow_code, amount FROM INDEMData\r\n"
			+ "    UNION ALL\r\n"
			+ "    SELECT loan_acct_no, flow_id, acct_name, flow_date, flow_code, amount FROM FEEDEMData\r\n" + ")\r\n"
			+ "SELECT \r\n" + "    aa.flow_date,\r\n" + "    aa.flow_id,\r\n" + "    aa.flow_code,\r\n" + "    CASE\r\n"
	...
	@Query(value = "WITH ControlDate AS (\r\n" + "    SELECT MAX(TRAN_DATE) AS TRAN_DATE \r\n"
			...
			+ "              AND flow_date < cd.TRAN_DATE\r\n" + "        )\r\n" + "),\r\n" + "\r\n"
			+ "FEEDEMData AS (\r\n" + "    -- Show only FEEDEM rows with flow_date = TRAN_DATE\r\n" + "    SELECT \r\n"
			+ "        aa.loan_acct_no,\r\n" + "        3 AS flow_id,\r\n" + "        aa.acct_name,\r\n"
			+ "        'FEEDEM' AS flow_code,\r\n" + "        aa.flow_date,\r\n"
			+ "        ISNULL(aa.FLOW_AMT, 0) AS amount\r\n" + "    FROM DEMAND_TBL aa\r\n"
			+ "    JOIN ControlDate cd ON 1 = 1\r\n" + "    WHERE\r\n" + "        aa.loan_acct_no = ?1\r\n"
			...
			+ "    SELECT loan_acct_no, flow_id, acct_name, flow_date, flow_code, amount FROM FEEDEMData\r\n" + ")\r\n"
			+ "\r\n" + "SELECT \r\n" + "    aa.flow_date,\r\n" + "    aa.flow_id,\r\n" + "    aa.flow_code,\r\n"
			+ "    ISNULL(aa.amount, 0) AS amount,\r\n" + "    aa.loan_acct_no,\r\n" + "    aa.acct_name,\r\n"
			+ "    cd.TRAN_DATE\r\n" + "FROM FinalResult aa\r\n" + "JOIN ControlDate cd ON 1 = 1\r\n" + "WHERE \r\n"
			+ "    (aa.flow_code IN ('PRDEM', 'INDEM') AND aa.flow_date != cd.TRAN_DATE)\r\n"
			+ "    OR (aa.flow_code = 'FEEDEM' AND aa.flow_date = cd.TRAN_DATE)\r\n" + "ORDER BY \r\n"
			+ "    aa.flow_date, \r\n" + "    CASE aa.flow_code \r\n" + "        WHEN 'INDEM' THEN 1\r\n"
			+ "        WHEN 'PRDEM' THEN 2\r\n" + "        WHEN 'FEEDEM' THEN 3\r\n" + "        ELSE 4 \r\n"
			+ "    END,\r\n" + "    aa.amount,\r\n" + "    aa.loan_acct_no,\r\n" + "    aa.acct_name\r\n"
			+ "", nativeQuery = true)
