...
public interface BLMS_PERSONALDET_REPO extends JpaRepository<BLMS_PERSONAL_LOAN_ENTITY, String>{
	...
	@Query(value = "SELECT * FROM BLMS_PERSONAL_LOAN_DETAILS WHERE verify_flg IN ('Y', 'N')", nativeQuery = true)
	List<BLMS_PERSONAL_LOAN_ENTITY> getApprovelist();
	
	@Query(value = "select * from BLMS_PERSONAL_LOAN_DETAILS  where verify_flg='Y' ", nativeQuery = true)
	List<BLMS_PERSONAL_LOAN_ENTITY> getapproved();
	
	@Query(value = "select * from BLMS_PERSONAL_LOAN_DETAILS  where verify_flg = 'N' ", nativeQuery = true)
